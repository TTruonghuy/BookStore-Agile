...
export default function Account() {
  ...
  function handleUpdateAddress(getCurrentAddress) {
    ...
    setAddressFormData({
      ...
      numberPhone: getCurrentAddress.numberPhone,
      ...
      <div className="mx-auto bg-gray-100 px-4 sm:px-6 lg:px-8">
        <div className="bg-white shadow">
          <div className="p-6 sm:p-12">
            ...
            <button onClick={()=>router.push('/orders')} className="mt-5  inline-block bg-gray-200 text-black px-5 py-3 text-xs font-medium uppercase tracking-wide">
              View Your Orders
            </button>
            <div className="mt-6">
                  ...
                  {addresses && addresses.length ? (
                    addresses.map((item) => (
                      <div className="border p-6" key={item._id}>
                        ...
                        <p>Number Phone : {item.numberPhone}</p>
                        <button
                          onClick={() => handleUpdateAddress(item)}
                          className="mt-5 mr-5 inline-block bg-gray-200 text-black px-5 py-3 text-xs font-medium uppercase tracking-wide"
                        >
                          Update
                        </button>
                        <button
                          onClick={() => handleDelete(item._id)}
                          className="mt-5  inline-block bg-gray-200 text-black px-5 py-3 text-xs font-medium uppercase tracking-wide"
                        >
                          {componentLevelLoader &&
                ...
                <div className="w-full mt-6 mr-0 mb-0 ml-0 space-y-8">
                  {addNewAddressFormControls.map((controlItem) => (
                    <InputComponent
                      ...
                      onChange={(event) =>
                        {
                          // Kiểm tra xem nếu đang nhập vào ô số điện thoại
                          if (controlItem.id === 'numberPhone') {
                            // Loại bỏ các ký tự không phải số từ giá trị nhập vào
                            const newValue = event.target.value.replace(/\D/g, '');
                            setAddressFormData({
                              ...addressFormData,
                              [controlItem.id]: newValue,
                            });
                          } else {
                            // Nếu không phải ô số điện thoại, chỉ cập nhật giá trị như bình thường
                            setAddressFormData({
                              ...addressFormData,
